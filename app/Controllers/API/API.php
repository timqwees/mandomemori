...
class API
{
   ...
   */
  private static function resolveJsonPath(string $file): ?string
  {
    $dataDir = dirname(__DIR__, 3) . '/storage/json';
    $realDataDir = realpath($dataDir);
    if ($realDataDir === false) {
      return null;
    }
    $clean = ltrim($file, '/');
    $resolved = realpath($realDataDir . '/' . $clean);
    if ($resolved === false || strpos($resolved, $realDataDir) !== 0) {
      return null;
    }
    return $resolved;
  }
  ...
  {
    $jsonPath = self::resolveJsonPath($file);
    if ($jsonPath === null) {
      http_response_code(400);
      header('Content-Type: application/json; charset=utf-8');
      return json_encode(['status' => 'error', 'data' => null, 'message' => 'Invalid file path', 'meta' => []], JSON_UNESCAPED_UNICODE);
    }

    //get data with file
    $data = json_decode(file_get_contents($jsonPath), true) ?: [];
    //first check method curl
    $isWriteMethod = in_array($_SERVER['REQUEST_METHOD'], ['POST', 'PUT', 'PATCH'], true);
    if ($isWriteMethod) {
      ...
      if (json_last_error() === JSON_ERROR_NONE && is_array($CURLOPT_POSTFIELDS)) {
        file_put_contents($jsonPath, json_encode(array_replace_recursive($data, $CURLOPT_POSTFIELDS), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
      }
    }
    ...
    try {
      if (!$isAPI) {
        ...
        $httpCode = 404;
      } elseif ($isWriteMethod) {//write request
