...
class Database extends Network
{
  ...
  private static function initSqliteFolder()
  {
    if (empty(self::$folder_sqlite)) {
      self::$folder_sqlite = dirname(__DIR__, 2) . "/storage/database/";
    }
  }
   ...
   * - Если переменная не задана, используется имя базы данных по умолчанию — `database`.
   *
   * ---
   * **Возвращает:**
   * `void`
  ...
  public static function initDatabaseName()
  {
    ...
    switch ($dbType) {
      case 'sqlite':
        self::$database_name = !empty($_ENV['SQLITE_DB_NAME']) ? $_ENV['SQLITE_DB_NAME'] : 'database';
        break;
      case 'mysql':
        self::$database_name = !empty($_ENV['DB_NAME']) ? $_ENV['DB_NAME'] : 'database';
        break;
      default:
